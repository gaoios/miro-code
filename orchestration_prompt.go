...
`)
	...
	if opencodeEnabled {
		writeWorkerRule(&b, "- **opencode — multi-model execution and verification worker**: use for bounded implementation, test execution, and independent verification when its capability and budget fit the task. Invoke with `soul spawn --bare --backend opencode --model <native-model-id> ...`, where `<native-model-id>` is the native provider/model ID from the `agents.opencode.model_map` configuration; model_map values pass through unchanged.", opencodeDispatchHint)
	}
	writeToolCLIRules(&b)
